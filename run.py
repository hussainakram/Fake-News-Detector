...
class Model(torch.nn.Module):
	def __init__(self, args):
		...
		self.nhidden = args.hidden_size
		self.num_classes = args.num_classes
		self.dropout_ratio = args.dropout_ratio
		self.model = args.model
		self.concat = False # set it True if linear transformation needed

		if self.model == 'gcn':
			self.conv1 = GCNConv(self.num_features, self.nhidden)
		elif self.model == 'sage':
			print('jdakjhsdjk')
			self.conv1 = SAGEConv(self.num_features, self.nhidden)
		elif self.model == 'gat':
			self.conv1 = GATConv(self.num_features, self.nhidden)

		if self.concat:
			self.lin0 = torch.nn.Linear(self.num_features, self.nhidden)
			self.lin1 = torch.nn.Linear(self.nhidden * 2, self.nhidden)

		self.lin2 = torch.nn.Linear(self.nhidden, self.num_classes)
...
parser.add_argument('--seed', type=int, default=777, help='random seed size')
parser.add_argument('--device', type=str, default='cpu', help='specify cuda devices')

# parameters for evaluation/comparison purpose
...
dataset = FNNDataset(root='data', selected_feature=args.feature, transform=ToUndirected())
...
if __name__ == '__main__':
	...
	for epoch in tqdm(range(args.epochs)):
		...
		for i, data in enumerate(train_loader):
			...
			likelihood_loss = F.nll_loss(out, y)
			likelihood_loss.backward()
			optimizer.step()
			loss_train += likelihood_loss.item()
			out_log.append([F.softmax(out, dim=1), y])
	...
	print(f'results: acc={acc:.4f}, f1_macro={f1_macro:.4f}, f1_micro={f1_micro:.4f}, '
